...
export class RolesController {
  ...
  async getByArrayIds(request: FastifyRequest, reply: FastifyReply) {
    const ids = arrayOfIdsSchema.parse(request.params);
    if (!ids) {
      return reply.status(400).send('Invalid Roles IDs');
    }
    
    const data = await this.service.findByArrayIds(ids);
